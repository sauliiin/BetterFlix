...
def get_ratings(imdb_id):
    ...
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.execute("SELECT data, timestamp FROM mdlist_data WHERE imdb_id = ?", (imdb_id,))
    ...
    try:
        ...
        for rating in json_data.get("ratings", []):
            ...
            # --- MUDANÇA AQUI ---
                ...
                # A nota do Trakt já vem em escala de 10, então não precisa de conversão.
                data_to_cache["trakt_rating"] = f"{value:.1f}" # Formata para 1 casa decimal
            # --- FIM DA MUDANÇA ---

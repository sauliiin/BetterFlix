...
def _get_best_image(images, allowed_langs):
    """Retorna a melhor imagem baseada em prioridade de idioma e votos"""
    if not images or not allowed_langs: return None
    valid_images = [img for img in images if img.get('iso_639_1') in allowed_langs]
    if not valid_images: return None
    priority_map = {lang: i for i, lang in enumerate(allowed_langs)}
    def sort_key(img):
        iso = img.get('iso_639_1')
        lang_rank = priority_map.get(iso, 999)
        votes = img.get('vote_count', 0)
        return (lang_rank, -votes) 
    try:
        valid_images.sort(key=sort_key)
        return valid_images[0].get('file_path')
    except: return None

def _get_all_images_by_priority(images):
    """Retorna todas as imagens organizadas por idioma em ordem de prioridade"""
    ...
    for img in images:
        ...
        if iso == 'pt-BR':
            result['pt-BR'].append((file_path, votes))
        elif iso == 'pt-PT':
            result['pt-PT'].append((file_path, votes))
        elif iso == 'pt':
            result['pt'].append((file_path, votes))
        elif iso in ('en-US', 'en'):
            result['en'].append((file_path, votes))
        else:
            result['other'].append((file_path, votes))
    
    # Ordena cada lista por votos (mais votos primeiro)
    for key in result:
        result[key].sort(key=lambda x: -x[1])
    
    return result

def enhance_details_with_best_images(details):
    if not details or not isinstance(details, dict): return details
    try:
        images_data = details.get('images', {})
        pt_langs = ['pt-BR', 'pt', 'pt-PT']
        en_langs = ['en-US', 'en', None, ''] 
        if 'posters' in images_data:
            # Armazena todas as opções de poster organizadas por idioma
            details['poster_options'] = _get_all_images_by_priority(images_data['posters'])
            # Mantém a compatibilidade com código antigo
            details['poster_path'] = _get_best_image(images_data['posters'], pt_langs)
            details['poster_path_fallback'] = _get_best_image(images_data['posters'], en_langs)
        if 'logos' in images_data:
            # Armazena todas as opções de clearlogo organizadas por idioma
            details['clearlogo_options'] = _get_all_images_by_priority(images_data['logos'])
            # Mantém a compatibilidade com código antigo
            details['clearlogo_path'] = _get_best_image(images_data['logos'], pt_langs)
            details['clearlogo_path_fallback'] = _get_best_image(images_data['logos'], en_langs)
    except: pass
    return details
...
def movie_details(tmdb_id, language, tmdb_api=None):
    try:
        url = '%s/movie/%s?api_key=%s&language=%s&append_to_response=%s&include_image_language=pt-BR,pt-PT,pt,null,en' % (base_url, tmdb_id, get_tmdb_api(tmdb_api), language, movies_append)
        details = get_tmdb(url)
        if details: details = enhance_details_with_best_images(details)
        return details
    except: return None

def tvshow_details(tmdb_id, language, tmdb_api=None):
    try:
        url = '%s/tv/%s?api_key=%s&language=%s&append_to_response=%s&include_image_language=pt-BR,pt-PT,pt,null,en' % (base_url, tmdb_id, get_tmdb_api(tmdb_api), language, tvshows_append)
        details = get_tmdb(url)
        if details: details = enhance_details_with_best_images(details)
        return details
    except: return None
